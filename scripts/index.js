...
async function makeGameScreen() {
  ...
  const createGameOverTable = (opts = {}) => {
    ...
    const tableLabelText = new PIXI.Text('Best score:'.toUpperCase(), {
      fontFamily: 'Arial',
      fontSize: 16,
      fontWeight: 'bold',
      fill: '#7c411e',
  ...
  const updateCookie = (cookie) => {
    if (cookie.y > app.screen.height + cookie.height) {
      removeCookie(cookie);
    }
  ...
  const createCookie = (cookieType) => {
    ...
    switch (cookieType) {
      case 0:
        cookieOpts.texures = [gameScreenAssets.cookie1];
        cookieOpts.scale = 0.75;
        cookieOpts.hp = 1;
        break;
      case 1:
        break;
